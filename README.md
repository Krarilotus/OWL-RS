...
- ontology-backed compat suites in the benchmark harness are now grouped under a dedicated `fixtures/compat/ontologies/` path, with additional vocabulary-specific parity cases for ORG, Time, SKOS, PROV-O, and DCAT on top of the shared ontology baseline suite
