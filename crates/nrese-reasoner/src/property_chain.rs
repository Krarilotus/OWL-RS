...
fn parse_chain(index: &IndexedDataset, head_id: u32) -> Result<Vec<u32>, &'static str> {
    ...
    loop {
        ...
        let property_id = single_list_value(index.list_first_by_node(), current_id, "rdf:first")?;
        chain.push(property_id);
        let next_id = single_list_value(index.list_rest_by_node(), current_id, "rdf:rest")?;
        if next_id == index.rdf_nil_id() {
            return Ok(chain);
        }
